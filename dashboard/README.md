...
```text
Jetson 검사
  └─ 원본·분석 JPEG + 녹·크랙 마스크 PNG + 최종 JSON
       ├─ 로컬 구성: 동기화된 inspection-export.json과 미리보기 표시
       └─ Firebase 구성(선택): Storage 파일 전송 → Firestore 실행 문서 게시
```
...
- Storage: `captures/**`에 원본·분석 JPEG, `dashboard/media/**`에 녹·크랙 마스크, `dashboard/manifests/**`에 최종 JSON 저장
- Jetson 업로더: 안전 종료 후 Storage 전송을 마친 다음 Firestore 문서 게시
...
- Jetson은 `device_uploaders/{UID}`에서 활성화된 전용 계정만 사용하며, 검사 자료 쓰기만 허용되고 읽기·삭제는 허용되지 않습니다.
- Storage의 `uploaderUid` 소유권 검사로 다른 Jetson 계정이 기존 객체를 덮어쓰지 못하게 제한합니다.
- Firestore와 Storage 보안 규칙이 실제 데이터 접근을 통제합니다.
- 원본 JPEG는 웹 공개 폴더에 넣지 않으며, 직접 업로드 사용 시 인증된 관리자만 읽을 수 있는 Storage 경로에 보관합니다.
