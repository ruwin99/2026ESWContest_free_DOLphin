...
*그림 1. 카메라·Jetson·STM32·주행부·클리닝 장치와 전원 연결 구성*
...
*그림 2. 실행 환경 준비부터 자동 임무, 실시간 ROI 분석, PWM 제어와 검사 결과 저장까지의 흐름*
...
*그림 3. 데이터 수집·학습·검증부터 ONNX 변환과 Jetson TensorRT 배포까지의 과정*
...
대시보드는 로컬 JSON/PNG 조회와 선택적 Jetson→Firebase 직접 업로드를 지원합니다. 클리너·펌프 안전 정지 요청과 자원 종료 절차를 거친 뒤 원본·분석 JPEG, 녹·크랙 마스크와 최종 JSON을 보호된 Storage에 올리고 Firestore에 실행 문서를 생성합니다. 원본 JPEG는 웹 공개 폴더에 넣지 않으며 대시보드는 로봇 제어 입력으로 사용하지 않습니다.
